...
function ApplicationDetailContent() {
  ...
  const [candidateProfile, setCandidateProfile] = useState<Profile | null>(
    null
  );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (applicationResponse.success && applicationResponse.data) {
          ...
          if (
            applicationResponse.data.user_profile &&
            applicationResponse.data.user_profile.id
          ) {
            try {
              const profileResponse = await getCandidateProfileById(
                applicationResponse.data.candidate_id
              );
          ...
          setError(
            applicationResponse.message || "Failed to fetch application"
          );
        }
      } catch (error) {
  ...
  const handleBackClick = () => {
    if (jobId && jobTitle) {
      router.push(
        `/company/applications?jobId=${jobId}&jobTitle=${encodeURIComponent(
          jobTitle
        )}`
      );
    } else {
  ...
  return (
    <div className="bg-neutral-light-40 min-h-screen py-4 px-10">
      ...
      <div className="flex flex-col">
        ...
        {candidateProfile && (
          <div className="bg-neutral-light-20 mb-6 py-4">
              ...
              <div className="flex flex-col gap-y-4 mx-10 my-4">
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-col border-t border-primary-40">
                    {application.application_status_history.map(
                      (history: ApplicationStatusHistory, idx: number) => (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          ...
          className="bg-secondary-60 hover:bg-secondary text-neutral-light-20 px-6 py-2 rounded-full cursor-pointer disabled:opacity-60"
          disabled={isUpdating}
        >
          Update Status
        </button>
        {showStatusForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 animate-fade-in">
            <form
              ...
              className="bg-white rounded-2xl shadow-2xl p-6 flex flex-col gap-4 w-full max-w-xs animate-fade-in"
            >
              ...
              <div className="flex flex-col gap-2">
                {STATUS_OPTIONS.map((opt) => (
                  <button
                    type="button"
                    key={opt.value}
                    className={`w-full text-left px-1 py-1 rounded-lg font-semibold border-none outline-none ${
                      selectedStatus === opt.value ? "ring-2 ring-accent" : ""
                    }`}
              ...
              <label className="font-semibold text-primary mt-2">
                Note (optional):
              </label>
              <textarea
